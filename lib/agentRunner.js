...
const ENDPOINTS = [
  { path: '/api/research/news',     label: 'News'     }, // Now points to Next.js API
  { path: '/api/research/academic', label: 'Academic' },
  { path: '/api/research/social',   label: 'Social'   },
  { path: '/api/research/tech',     label: 'Tech'     },
  { path: '/api/research/wiki',     label: 'Wiki'     },
  { path: '/api/research/crypto',   label: 'Crypto'   },
  { path: '/api/research/quotes',   label: 'Quotes'   },
  { path: '/api/research/meteo',    label: 'Meteo'    },
];
...
export async function executeResearch({ topic, priority = 'Medium', maxRetries = 2, bypassCode = '', origin = '', log = console.log }) {
  ...
  const baseUrl = origin || process.env.NEXT_PUBLIC_APP_URL || `http://localhost:${PORT}`;
