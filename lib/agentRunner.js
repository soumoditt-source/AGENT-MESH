...
export async function executeResearch({ topic, priority = 'Medium', maxRetries = 2, bypassCode = '', log = console.log }) {
  ...
  for (let i = 0; i < queries.length; i++) {
    ...
    log(`Sub-Agent ${i + 1}/${queries.length}: "${query}" → ${endpoint.label}`);
    ...
    try {
      ...
      log(`✓ Sub-Agent ${i + 1} got ${resultCount} results from ${data.source || endpoint.label}`);
    } catch (err) {
      log(`✗ Sub-Agent ${i + 1} failed: ${err.message}`);
