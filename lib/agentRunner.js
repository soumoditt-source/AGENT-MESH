...
const ENDPOINTS = [
  { path: '/research/news',     label: 'News'     },
  { path: '/research/academic', label: 'Academic' },
  { path: '/research/social',   label: 'Social'   },
  { path: '/research/tech',     label: 'Tech'     },
  { path: '/research/wiki',     label: 'Wiki'     },
  { path: '/research/crypto',   label: 'Crypto'   },
  { path: '/research/quotes',   label: 'Quotes'   },
];
 ...
 */
export async function executeResearch({ topic, priority = 'Medium', maxRetries = 2, bypassCode = '', log = console.log }) {
  ...
  // Query 1 -> News & Tech
  // Query 2 -> Academic & Wiki
  // Query 3 -> Social & Crypto
  const researchPlan = [
    ...
    { query: queries[2], endpoints: [ENDPOINTS[2], ENDPOINTS[5]] }, // Social, Crypto
  ];

  log(`Executing Max Research Plan across ${researchPlan.reduce((acc, p) => acc + p.endpoints.length, 0)} intelligence nodes...`);

  for (let i = 0; i < researchPlan.length; i++) {
    for (const endpoint of researchPlan[i].endpoints) {
      const query = researchPlan[i].query;
      log(`Agent-[${endpoint.label}]: Searching "${query}"...`);

      try {
        const { data, txHash, paid, amount } = await fetchWithX402(baseUrl, endpoint.path, query, maxRetries, bypassCode);
        ...
        log(`✓ Node-[${endpoint.label}] delivered ${resultCount} data points`);
      } catch (err) {
        log(`✗ Node-[${endpoint.label}] failed: ${err.message}`);
  ...
  let reportText;
  try {
    log('Synthesizing report with Gemini AI...');
    reportText = await synthesizeReport(topic, allResults);
    log('✓ Report synthesized');
  } catch (e) {
    log(`Synthesis error: ${e.message}`);
    reportText = allResults
      .filter(r => !r.error)
