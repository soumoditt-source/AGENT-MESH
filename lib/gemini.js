...
Keep each query 2-6 words, highly specific and actionable.`;
  ...
  if (queries.length === 0) {
    return [topic, `${topic} analysis`, `${topic} latest news`].slice(0, 3);
  }
...
Final Instruction: Be verbose where data allows, but stay objective. The report must be a comprehensive 800-1200 word clinical intelligence briefing.`;
  ...
  try {
    ...
    // Generate a high-quality manual collation as the ultimate fallback
    const reportText = [
      `## 🔍 Intelligence Collation (Fallback Mode)`,
      `The autonomous synthesis engine encountered a processing bottleneck. Raw intelligence from ${validResults.length} nodes follow:`,
      validResults.length > 0 
        ? validResults.map(r => `### Node: ${r.source}\n- Query: ${r.query}\n- Key Data: ${JSON.stringify(r.results[0]).slice(0,300)}...`).join('\n\n')
        : `No structured data was retrieved from the research nodes for this topic. Check connectivity and API keys.`
    ].join('\n\n');
    return reportText;
  }
}
