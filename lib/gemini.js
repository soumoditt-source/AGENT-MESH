...
async function callLLM(prompt, level = 'high') {
  ...
  try {
    ...
    const result = await model.generateContent({
      ...
      // Ultra-Advanced Thinking Configuration
      thinkingConfig: {
...
- CRITICAL: Cite real headlines, subreddit names, arxiv paper IDs, coin metrics, and specific weather conditions from the actual data provided above.
- CROSS-SOURCE CORRELATION: Explicitly link findings across nodes. For example, correlate a news headline with social sentiment or market impact.
- If data is missing for a section, provide a strategic "Data Gap Analysis" explaining what is needed or why the node returned limited results.
...
Analyze Reddit and HackerNews discussions. Mention subreddit names, upvote ratios, or common community arguments.

## 💰 Economic & Market Impact (Crypto/Finance)
If relevant data exists from CoinCap, synthesize the market implications. Include symbol performance or market caps.
