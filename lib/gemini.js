...
Keep each query 2-6 words, highly specific and actionable.`;
  ...
  let text = '';
  try {
    text = await callLLM(prompt, 'low');
  } catch (e) {
    console.error(`[AI] Sub-query generation failed: ${e.message}`);
  }

  const queries = (text || '').split('\n').map(q => q.trim()).filter(q => q.length > 3);
  const finalQueries = queries.slice(0, 3);
  while (finalQueries.length < 3) {
    const fallbacks = [`${topic} deep dive`, `${topic} news`, `${topic} analysis`].filter(f => !finalQueries.includes(f));
    finalQueries.push(fallbacks[0] || `${topic} research ${finalQueries.length}`);
  }
  return finalQueries;
}
...
- Cite real headlines, reddit threads, arxiv paper IDs, and coin metrics from the data provided.
- FORMAT: STRICTLY use professional Markdown headers.
- If data is sparse, explain the "Intelligence Gaps" but still provide a quality briefing.
...
Final Instruction: Be verbose where data allows, but stay objective. The report must be a comprehensive 800-1200 word clinical intelligence briefing.`;
  ...
  try {
    console.log(`[AI] Initiating neural synthesis for "${topic}"`);
    const report = await callLLM(prompt, 'high');
    
    if (!report || report.trim().length < 50) {
      throw new Error('Neural synthesis resulted in insufficient intelligence payload.');
    }
    return report;
  } catch (e) {
    console.error(`[AI] Synthesis Critical Failure: ${e.message}`);
    const summary = validResults.length > 0 
      ? validResults.map(r => `### Node: ${r.source}\n- Status: ${r.results.length} items retrieved\n- Key Intelligence: ${JSON.stringify(r.results[0] || "No snippet available").slice(0,250)}...`).join('\n\n')
      : `No data was successfully retrieved from the 8 intelligence nodes for "${topic}".`;

    return `## 🔍 Intelligence Collation (Direct Context Extraction)
The autonomous synthesis engine encountered a processing bottleneck. Raw intelligence extraction for "${topic}" follow:

${summary}

---
*Status: Fallback Intelligence Mode Active*`;
  }
}
